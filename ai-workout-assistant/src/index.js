...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Check URL for exercise parameter
  const getExerciseFromURL = () => {
  ...
  const getHTMLChooseWO = (data, isSettings) => {
    ...
    const fieldsetStart = `<fieldset class="grid grid-cols-2 gap-3 w-full">`;
    const fieldsetEnd = `</fieldset>`;
    htmlChooseWO += fieldsetStart;

    // If coming from URL with exercise param, only show that exercise
    if (exerciseFromURL && !isSettings) {
      const matchingIndexes = [];

      // Find the matching exercise(s)
      data.nameWorkout.forEach((nameWO, idx) => {
        const slug = data.slugWorkout[idx];
        if (
          (exerciseFromURL === 'pushups' && (slug === 'pushups' || slug === 'push-ups')) ||
          (exerciseFromURL === 'squats' && slug === 'squats')
        ) {
          matchingIndexes.push(idx);
        }
      });

      // If found matching exercises, only show those
      if (matchingIndexes.length > 0) {
        matchingIndexes.forEach(idx => {
          htmlChooseWO += `
            <label
              for="${isSettings ? `settingsName${idx}` : `chooseName${idx}`}"
              class="flex cursor-pointer items-center pl-4 border border-gray-200 rounded-lg col-span-2"
            >
              <input
              ...
              <span class="w-full py-4 ml-2 text-sm font-medium text-gray-600"
                >${data.nameWorkout[idx]}</span
              >
            </label>
          `;
        });
      } else {
        // Fallback to showing all exercises if no match found
        data.nameWorkout.forEach((nameWO, idx) => {
          htmlChooseWO += `
            <label
              for="${isSettings ? `settingsName${idx}` : `chooseName${idx}`}"
      ...
      // Show all exercises for settings page or when no URL parameter
      data.nameWorkout.forEach((nameWO, idx) => {
        htmlChooseWO += `
          <label
            for="${isSettings ? `settingsName${idx}` : `chooseName${idx}`}"
    ...
    htmlChooseWO += fieldsetEnd;

    htmlChooseWO += `<div class="${isSettings ? "mt-3" : "mt-5"
      } mb-3">How long?</div>`;

    htmlChooseWO += `<fieldset class="grid grid-cols-2 gap-3 w-full">`;

    data.duration.forEach((duration, idx) => {
      const isFirstDuration = idx === 0;
      htmlChooseWO += `
        <label
          for="${isSettings ? `settingsDuration${idx}` : `chooseDuration${idx}`
        }"
          class="flex cursor-pointer items-center pl-4 border border-gray-200 rounded-lg"
        >
          <input
            id="${isSettings ? `settingsDuration${idx}` : `chooseDuration${idx}`
        }"
            type="radio"
            value="${duration}"
            name="${isSettings ? "settingsDurationWO" : "chooseDurationWO"}"
            class="w-4 h-4 text-yellow-600"
            ${exerciseFromURL && isFirstDuration && !isSettings ? 'checked' : ''}
            required
          />
          <span class="w-full py-4 ml-2 text-sm font-medium text-gray-600"
            >${duration}</span
  ...
  // Automatically select workout based on URL parameter
  const autoSelectWorkout = (data) => {
    const exerciseName = getExerciseFromURL();
    if (exerciseName) {
      // Since we've already set the radio buttons to be checked in the HTML generation,
      // we just need to click the submit button after a short delay to allow the DOM to be ready
      setTimeout(() => {
        const submitBtn = document.getElementById('submitWOBtn');
        if (submitBtn) {
          submitBtn.click();
        }
      }, 500);
  ...
  const setupWOScore = async (path) => {
    return await fetch(path)
      ...
      .then(async (data) => {
        ...
        return data; // Return data for use in the then() handler
      })
      .catch((e) => {
        console.error(e);
        return null;
  ...
  await setupWOScore("./mock-data/workout.json").then(data => {
    if (data) {
      autoSelectWorkout(data);
    }
  });
  ...
  formChooseWOElem.addEventListener("submit", async (event) => {
    ...
    // If this was triggered programmatically by autoSelectWorkout, don't interfere
    if (event.isTrusted === false && getExerciseFromURL()) {
      return;
    }
    ...
    if (event.submitter && event.submitter.id === "submitWOBtn") {
